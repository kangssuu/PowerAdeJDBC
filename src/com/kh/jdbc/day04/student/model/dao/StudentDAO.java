...
public class StudentDAO {
	 ...
	 * 3. 예외처리 처리 방법 : throws, try ~ catch문
	 */
	...
	public StudentDAO() {
		...
		try {
			...
			prop.load(reader);
		} catch (Exception e) {  // 예외처리에도 순서가 있음. 맨 처음에 부모Exception을 적으면 밑에 예외코드(자식예외코드)들은 실행 안됨
			// Exception => 최상위 예외처리 문
			e.printStackTrace();
...
//		}
	...
	public List<Student> selectAll(Connection conn) throws SQLException {  // throws 예외 넘김(던짐)
		Statement stmt = null;
		ResultSet rset = null;
...
//			rset = stmt.executeQuery(query);
	...
	private Student rsetToStudent(ResultSet rset) throws SQLException {
		...
		student.setEnrollDate(rset.getDate("ENROLL_DATE"));  // getDate => 데이터 타입에 맞게 가져와야함
		return student;
	}
}
